- [Introduction](/home.md ':class=section-title')

- [Contribution](/contribution.md ':class=section-title')

- [Best Practices](best-practices/intro/intro.md ':class=section-title')
  - [Markup](best-practices/markup/markup.md?id=markup ':class=section-title')
  - [CSS](best-practices/css/css?id=css ':class=section-title')
  - [PHP](best-practices/php/php.md?id=php ':class=section-title')
  - [Accessibility](best-practices/a11y/a11y.md ':class=section-title')

- Dev Workflows
  - [Build Process](/local-development/build-process.md ':class=section-title')
  - [Local Dev Env](/local-development/setup-local-dev.md ':class=section-title')
  - [WordPress Dev Workflow](/local-development/wordpress-dev-workflow.md ':class=section-title')
  - [Tools](/local-development/tools.md ':class=section-title')
